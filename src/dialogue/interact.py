...
def sample_sequence(history, tokenizer, model, args, current_output=None):
    ...
    for i in range(args.max_length):
        instance = Instance(history , current_output, with_eos=False)
        inputs = instance.get_reply(tokenizer)
        ...
        token_type_ids = torch.tensor(inputs["token_type_ids"], device=args.device).unsqueeze(0)
